...
vector<double> getParameterValues(int domainDim, const vector<vector<double>>& knotVectors,
    ...
    for (int d = 0; d < domainDim; ++d) {
        ...
        double end = kv[lengths[d]];
        // Distribute test points evenly across parameter space
        double t = start + (end - start) * (testIndex + 1) / (totalTests+1.0);
        params[d] = t;
    }
    return params;
}
...
// Test function for domain dimension 1
template<typename RangeType>
double testMapping1D(int rangeDim, int testCount) {
    vector<SInt> orders(1, 4);
    vector<SInt> lens(1, 512);
    int total_points = lens[0];
    ...
    // Pre-generate all test parameters
    vector<Pnt1D> testParams;
    for (int t = 0; t < testCount; ++t) {
        vector<double> params = getParameterValues(1, bspline.knotVectors(),
            bspline.orders(), bspline.lengths(), t, testCount);
        testParams.push_back(createPoint<Pnt1D>(params));
    }

    // Time only the evaluate() calls
    auto start = chrono::high_resolution_clock::now();
    for (int t = 0; t < testCount; ++t) {
        RangeType result = bspline.evaluate(testParams[t]);
    }
    auto end = chrono::high_resolution_clock::now();
...
// Test function for domain dimension 2
template<typename RangeType>
double testMapping2D(int rangeDim, int testCount) {
    vector<SInt> orders(2, 5);
    vector<SInt> lens(2, 256);
    int total_points = lens[0] * lens[1];
    cout << "total_points: " << total_points << endl;
    vector<RangeType> pts;
    ...
    // Pre-generate all test parameters
    vector<Pnt2D> testParams;
    for (int t = 0; t < testCount; ++t) {
        vector<double> params = getParameterValues(2, bspline.knotVectors(),
            bspline.orders(), bspline.lengths(), t, testCount);
        testParams.push_back(createPoint<Pnt2D>(params));
    }
...
// Test function for domain dimension 3
template<typename RangeType>
double testMapping3D(int rangeDim, int testCount) {
    vector<SInt> orders(3, 5);
    vector<SInt> lens(3, 128);
    int total_points = lens[0] * lens[1] * lens[2];
    cout << "total_points: " << total_points << endl;
    vector<RangeType> pts;
    ...
    // Pre-generate all test parameters
    vector<Pnt3D> testParams;
    for (int t = 0; t < testCount; ++t) {
        vector<double> params = getParameterValues(3, bspline.knotVectors(),
            bspline.orders(), bspline.lengths(), t, testCount);
        testParams.push_back(createPoint<Pnt3D>(params));
    }
...
void runAllTests() {
    ...
    cout << "Testing all 9 domain->range combinations\n";
    cout << "Tests per mapping: " << testsPerMapping << "\n";
    cout << "Timing: evaluate() calls ONLY\n";
    cout << "========================================\n\n";
    ...
    for (int rangeDim = 1; rangeDim <= 3; ++rangeDim) {
        ...
        double evalTime = 0.0;
        if (rangeDim == 1) {
            evalTime = testMapping1D<Pnt1D>(1, testsPerMapping);
        }
        else if (rangeDim == 2) {
            evalTime = testMapping1D<Pnt2D>(2, testsPerMapping);
        }
        else {
            evalTime = testMapping1D<Pnt3D>(3, testsPerMapping);
        }

        totalTests += testsPerMapping;
        totalEvalTime += evalTime;
        cout << "  Completed " << testsPerMapping << " tests\n";
        cout << "  evaluate() time: " << evalTime << " seconds\n";
        cout << "  Avg per evaluate(): " << (evalTime / testsPerMapping) << " seconds\n\n";
    }
    ...
    for (int rangeDim = 1; rangeDim <= 3; ++rangeDim) {
        ...
        double evalTime = 0.0;
        if (rangeDim == 1) {
            evalTime = testMapping2D<Pnt1D>(1, testsPerMapping);
        }
        else if (rangeDim == 2) {
            evalTime = testMapping2D<Pnt2D>(2, testsPerMapping);
        }
        else {
            evalTime = testMapping2D<Pnt3D>(3, testsPerMapping);
        }
    ...
    for (int rangeDim = 1; rangeDim <= 3; ++rangeDim) {
        ...
        double evalTime = 0.0;
        if (rangeDim == 1) {
            evalTime = testMapping3D<Pnt1D>(1, testsPerMapping);
        }
        else if (rangeDim == 2) {
            evalTime = testMapping3D<Pnt2D>(2, testsPerMapping);
        }
        else {
            evalTime = testMapping3D<Pnt3D>(3, testsPerMapping);
        }
    ...
    cout << "========================================\n";
    cout << "Total tests executed: " << totalTests << "\n";
    cout << "Total evaluate() time: " << totalEvalTime << " seconds\n";
    cout << "Average per evaluate(): " << (totalEvalTime / totalTests) << " seconds\n";
    cout << "========================================\n";
}
...
int main() {
    ...
    chrono::duration<double> elapsed = end - start;
    cout << "\nTotal execution time (including setup): " << elapsed.count() << " seconds\n\n";
